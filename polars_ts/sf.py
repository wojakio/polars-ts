...
class SeriesFrame:
    ...
    def __init__(self, df: pl.LazyFrame):
        self._df = (
            df
            .with_columns(pl.lit(0, pl.Boolean).alias(self._RESERVED_ALL_GRP))
        )
    ...
    def categories(self) -> pl.LazyFrame:
        return (
            self._df
            .select(
                pl.col(pl.Categorical, pl.Enum)
            )
        )
    
    def ht(self, nrows=3) -> pl.LazyFrame:
        if len(self._df) <= (nrows + nrows): return self._df
        return (pl.concat([
            self._df.head(nrows),
            self._df.tail(nrows)
        ]))
    ...
    def common_category_names(self, rhs: pl.LazyFrame) -> List[str]:
        return sorted(
            set(self.category_names())
            .intersection(rhs.tsf.category_names())
        )
