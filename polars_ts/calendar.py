...
class CalendarFrame(SeriesFrame):
    ...
    def date_to_imm_contract(
        ...
        contract_code = pl.lit(contract_code) if isinstance(contract_code, str) else contract_code
    ...
    def imm_contract_to_date(self, col_name: str, default_day: int = 1) -> pl.Expr:
        ...
        self._df = (
            ...
            .with_columns(
                ...
                .map_elements(
                    lambda se: (f"{str(default_day).rjust(2, '0')}-{str(imm_dict[se['imm']]).rjust(2, '0')}-{se['year']}"),
                    return_dtype=pl.String
                )
                .str.to_date("%d-%m-%y")
            )
