...
def impl_resample_categories(
    ...
    new_index = (pl
        .LazyFrame({"time": time_axis})
        .join(categories, how='cross')
    )
    ...
    if closed != 'none':
        start_ends = (
            ...
            .select(
                *partition,
                start=pl.max_horizontal(pl.col("time").min().over(partition), time_axis.min()),
                end=pl.min_horizontal(pl.col("time").max().over(partition), time_axis.max()),
            )
...
def impl_align_values(
    ...
    result = (lhs
        .join(rhs, on=["time"] + partition, how='outer_coalesce')
