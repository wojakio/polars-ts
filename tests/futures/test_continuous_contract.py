...
def test_continuous_contract():
    ...
    generic_symbols = generic_symbol_universe(roll_config, market_start_date, market_end_date)
    ...
    instrument_prices = (
        security_dates
        .dummymkt.fetch_instrument_prices(
            "fut_first_trade_dt",
    ...
    result_schema = list(result.schema.items())

    expected_schema = [
        ('time', pl.Date),
        ('asset', pl.Categorical(ordering='physical')),
        ('instrument_id', pl.Struct({'tenor': pl.Categorical(ordering='physical'), 'year': pl.Int32})),
